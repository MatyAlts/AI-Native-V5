#!/usr/bin/env bash
# Variante de dev-start-all.sh que invoca los binarios del venv directamente
# (sin pasar por `uv run`) para evitar que uv resuelva el lock para todos los
# environments del workspace (que falla por mistralai quarantined en PyPI).
set -euo pipefail
cd "$(dirname "$0")/.."
...
start_svc() {
  ...
  "$UVICORN" "${module}:app" --host 127.0.0.1 --port "${port}" --log-level info \
    > "${log}" 2>&1 &
...
start_ctr_worker() {
  ...
  "$PY" -m ctr_service.workers.partition_worker --partition "${partition}" \
    > "${log}" 2>&1 &
