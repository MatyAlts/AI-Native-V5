...
Reemplaza el path legacy `ops/grafana/` (archivado en `_archive/` con README de
deprecación) que tenía 2 dashboards aspiracionales — referenciaban métricas que
nunca se emitieron porque no había `MeterProvider` configurado.

Política de cardinalidad acotada por diseño: las labels permitidas son una lista
cerrada documentada en el README operativo del directorio. `student_pseudonym`,
...
Las labels prohibidas SHALL JAMÁS aparecer en métricas Prometheus emitidas por servicios del piloto: `student_pseudonym`, `episode_id`, `user_id`, `prompt_id`, ni cualquier UUID per-instancia.
