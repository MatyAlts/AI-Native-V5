...
class TutorCore:
    ...
    # ── Reapertura docente de un episodio cerrado (2026-06-19) ──────────
    ...
    async def reopen_episode(
        ...
        tenant_id: UUID,
        docente_id: UUID,
        docente_roles: frozenset[str],
        razon: str = "",
    ) -> dict:
        """Reabre un episodio CERRADO para que el alumno lo retome (acción docente).

        A diferencia de `resume_episode` (que reconstruye la sesión Redis del
        propio alumno), esto NO reconstruye nada: solo emite `episodio_reabierto`
        al CTR, que vía el partition_worker revierte `estado` closed→open. La
        reconstrucción de la sesión ocurre cuando el alumno vuelve a la TP — su
        flujo normal (`resume_episode`, que ya acepta `open`) levanta el episodio.
        Append-only: la cadena criptográfica continúa, sin mutar eventos previos.

        Autorización: solo un docente de la comisión del episodio (o superadmin)
        puede reabrirlo. En prod el tenant es compartido, así que validamos
        pertenencia contra `usuarios_comision` (academic-service), NO por tenant.

        Returns:
            dict con episode_id, problema_id, comision_id y estado="open".

        Raises:
            HTTPException 404 — episodio inexistente.
            HTTPException 403 — episodio de otro tenant, o docente que no
              pertenece a la comisión (y no es superadmin).
            HTTPException 409 — el episodio no está cerrado.
        """
        ep = await self.ctr.get_episode(episode_id, tenant_id, TUTOR_SERVICE_USER_ID)
        if ep is None:
            raise HTTPException(
        ...
        if estado != "closed":
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"Solo se reabren episodios cerrados (estado actual: '{estado}')",
            )

        comision_id = UUID(str(ep["comision_id"]))
        problema_id = UUID(str(ep["problema_id"]))

        # Authz de pertenencia: el docente debe estar asignado a la comisión del
        # episodio (salvo superadmin, que opera cross-comisión). Fail-closed: si
        # no figura en usuarios_comision, 403.
        if "superadmin" not in docente_roles:
            docentes = await self.academic.list_docentes_comision(
                comision_id=comision_id,
                tenant_id=tenant_id,
                caller_id=TUTOR_SERVICE_USER_ID,
            )
            asignados = {
                str(d.get("user_id")) for d in docentes if d.get("user_id") is not None
            }
            if str(docente_id) not in asignados:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="El docente no pertenece a la comisión de este episodio",
                )

        # Construir el evento episodio_reabierto. Sin sesión viva (el cierre la
        # borró), tomamos los hashes/versión del ÚLTIMO evento de la cadena para
        # mantener consistencia con el resto del episodio. seq = events_count.
        events: list[dict] = sorted(ep.get("events") or [], key=lambda e: e.get("seq", 0))
        if not events:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Episodio sin eventos persistidos — no se puede reabrir",
            )
        last_ev = events[-1]
        reopen_event = {
            ...
            "seq": int(ep["events_count"]),
            "event_type": "episodio_reabierto",
            "ts": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
            "payload": {"docente_id": str(docente_id), "razon": razon},
            "prompt_system_hash": last_ev["prompt_system_hash"],
            "prompt_system_version": last_ev["prompt_system_version"],
            "classifier_config_hash": last_ev["classifier_config_hash"],
        }
        await self.ctr.publish_event(reopen_event, tenant_id, TUTOR_SERVICE_USER_ID)

        logger.info(
            "episodio reabierto por docente episode_id=%s docente=%s comision=%s",
            episode_id,
            docente_id,
            comision_id,
        )
