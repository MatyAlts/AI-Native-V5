...
- **Hueco conocido — arrastrar y soltar**: `dropIntoEditor` de Monaco queda en su default (`true`) y no hay listener de `drop`/`dragover`. Texto arrastrado desde otra ventana entra al buffer y sale como `origin: "student_typed"`. El valor `drag_drop` del campo `metodo` de `pega_intentada` está declarado en los 6 archivos de la pila y no lo emite nadie. Cerrarlo cambia qué eventos entran al CTR — decisión pendiente del equipo, no del editor.
