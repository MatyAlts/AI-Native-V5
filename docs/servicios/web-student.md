...
- Detectar el origen de cada `edicion_codigo` y propagarlo en el campo `origin` (F6): `student_typed` (default) vs `snippet_expanded` (expansión de un snippet de ceremonia del editor). **`pasted_external` NO se emite hoy y no es un bug**: el editor BLOQUEA el pegado por dos vías independientes (el keybinding `Ctrl+V` reemplazado y un listener DOM `paste` en fase de captura con `preventDefault()` + `stopPropagation()`), así que el `onDidPaste` de Monaco —único setter de esa marca— nunca se dispara. Un intento de pegar sale como evento CTR `pega_intentada`, no como un `edicion_codigo` marcado. El valor `copied_from_tutor` está reservado para cuando exista un botón "insertar al editor" en el chat del tutor (no implementado aún, ADR-026). Permite distinguir delegación pasiva sin depender solo de inferencia temporal.
- **Arrastrar y soltar: cerrado el 2026-08-28.** Fue un hueco real mientras duró: `dropIntoEditor` de Monaco quedaba en su default (`true`) y no había listener de `drop`/`dragover`, así que el texto arrastrado desde otra ventana entraba al buffer y salía como `origin: "student_typed"` — la plataforma afirmando que el alumno tipeó código que no tipeó. Se cerró con el mismo criterio que el clipboard: se bloquea y se registra el intento. Son **dos mitades y hacen falta las dos** — `dropIntoEditor: { enabled: false }` apaga el widget de Monaco, y los listeners `dragover`/`drop` en fase de captura impiden que el navegador inserte el texto por su cuenta; cancelar el `dragover` es lo que le dice al navegador que ahí no se suelta. El evento sale como `pega_intentada` con `metodo: "drag_drop"`, un valor que **ya estaba declarado** en los 6 archivos de la pila y que simplemente no emitía nadie: no se agregó un tipo de evento nuevo a la cadena. Cubierto por `tests/CodeEditorArrastre.test.tsx`, donde cada test mata una mutación distinta (incluida la de emitir en `dragover`, que inflaría la cadena con un evento por cada píxel del arrastre).
