...
Queda además un canal abierto que ninguna de las dos defensas cubre: **arrastrar y soltar** texto desde otra ventana. Monaco lo acepta (`dropIntoEditor`, default `true`) y el componente no tiene listener de `drop`/`dragover`, así que el texto entra al buffer y sale como `origin: "student_typed"`. El valor `drag_drop` del campo `metodo` de `pega_intentada` está declarado en los 6 archivos de la pila y no lo emite nadie.
...
>
> Se documenta además un canal residual que ninguna de las dos defensas cubre: el arrastrar-y-soltar de texto desde otra ventana del sistema, que el editor acepta por su configuración por omisión y que ingresa al buffer sin distinguirse del tipeo. Su cierre se difiere porque altera qué eventos ingresan al registro y no puede introducirse mid-cohort sin romper la comparabilidad entre cohortes.
