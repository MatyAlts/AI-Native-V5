...
> **Corrección 2026-08-28.** Este apartado decía que el frontend "sólo emite `student_typed | pasted_external`". Es falso, y el error es en la dirección peligrosa: **`pasted_external` tampoco se emite**. Lo que sigue está reescrito contra el código.

El frontend `web-student` emite **`student_typed` y `snippet_expanded`**, y ninguno de los dos valores que llevan override a N4.

- `copied_from_tutor` requiere un botón "Insertar código" en el panel del tutor que no está implementado (ADR-026 lo difiere). Verificación: `rg "copied_from_tutor" apps/web-student` → sólo aparece como type literal de TypeScript, no como string emitido en runtime.
- `pasted_external` se marca únicamente desde `editor.onDidPaste` en `CodeEditor.tsx`, y **ese callback no se dispara nunca**: el mismo componente bloquea el pegado por dos vías independientes que corren antes — el keybinding `Ctrl+V` reemplazado con `editor.addCommand`, y un listener DOM `paste` en fase de **captura** con `preventDefault()` + `stopPropagation()` sobre el contenedor, que corta el evento antes de que llegue al textarea oculto que Monaco escucha. No es una rama rota sino deliberadamente vacía: el editor no *trackea* el pegado, lo *prohíbe*, y en su lugar emite el evento CTR `pega_intentada`.

El event_labeler (ADR-020) sí reconoce los tres valores y aplica override a N4 para `copied_from_tutor` y `pasted_external` — pero **ninguno de los dos llega al CTR desde el editor del alumno**. La consecuencia pedagógica no es la que decía este documento (una métrica colapsada en `pasted_external`) sino una más fuerte: **el override a N4 por `origin` no se dispara nunca en el piloto**, y la apropiación de código externo se infiere sólo por vía temporal.

Había además un tercer canal que ninguna de las dos defensas cubría: **arrastrar y soltar** texto desde otra ventana. Monaco lo aceptaba (`dropIntoEditor`, default `true`) y el componente no tenía listener de `drop`/`dragover`, así que el texto entraba al buffer y salía como `origin: "student_typed"`. **Se cerró el 2026-08-28** con el mismo criterio que el clipboard —bloquear y registrar el intento—, y el cierre resultó menos invasivo de lo que se temía: el valor `drag_drop` del campo `metodo` de `pega_intentada` **ya estaba declarado** en los 6 archivos de la pila y no lo emitía nadie, así que empezar a emitirlo no introduce un tipo de evento nuevo en el registro.

### Propuesta de redacción (19.5 — fragmento sobre `origin` de `edicion_codigo`)

> El payload del evento `edicion_codigo` incluye desde F6 un campo opcional `origin: Literal["student_typed", "copied_from_tutor", "pasted_external"] | None` que distingue tres procedencias del cambio en el editor del estudiante. El etiquetador (ADR-020) aplica override `→ N4` para `copied_from_tutor` y `pasted_external` — interpretándolos pedagógicamente como acciones que provienen de una interacción IA o externa, no de elaboración propia. **En la versión v1.0.x del piloto ninguno de esos dos valores se emite en runtime**, de modo que el override a `N4` por `origin` no se activa: el editor emite `student_typed` (tipeo directo en Monaco) y `snippet_expanded` (expansión de una plantilla de ceremonia del propio editor), ambos sin override. Las razones son distintas para cada uno y conviene no confundirlas. `copied_from_tutor` está declarado en el contrato y reconocido por el etiquetador, pero requiere una afordancia UX —un botón "Insertar código" junto a cada bloque de código en las respuestas del tutor— que no está implementada; su introducción mid-cohort modificaría la condición experimental, y por eso está diferida a post-defensa (ADR-026, ver también confound 11.6). `pasted_external`, en cambio, es consecuencia directa de una decisión de diseño anterior: **el editor no registra el pegado, lo impide** — reemplaza el atajo de teclado e intercepta el evento `paste` del DOM en fase de captura— y en su lugar emite un evento independiente, `pega_intentada`, que deja constancia del intento sin insertar el contenido. La detección de procedencia externa dentro de `edicion_codigo` queda por lo tanto sin uso en esta versión, y la apropiación de código ajeno se infiere sólo por vía temporal.
>
> Un tercer canal —el arrastrar-y-soltar de texto desde otra ventana del sistema— ingresaba al buffer sin distinguirse del tipeo, porque el editor lo aceptaba por su configuración por omisión. Se cerró bajo el mismo principio que el portapapeles: la acción se impide y el intento se registra. El cierre no altera la comparabilidad entre cohortes, dado que el valor correspondiente del registro ya formaba parte del contrato del evento y sólo pasó a tener emisor; los episodios anteriores al cierre se distinguen por la ausencia de ese valor, no por un esquema distinto.

### Coordinación con código

- ADR-026 — decisión de DIFERIR; criterios de re-evaluación (cohorte limpia o estudio cuasi-experimental con grupo control).
- `apps/web-student/src/components/CodeEditor.tsx` — emite `student_typed` y `snippet_expanded`; el `onDidPaste` que marcaría `pasted_external` es inalcanzable por el bloqueo de clipboard del mismo archivo (comentado ahí en detalle).
- `apps/classifier-service/src/classifier_service/services/event_labeler.py` — override reconoce los tres; en el piloto no se dispara ninguno.
