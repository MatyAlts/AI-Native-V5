...
async def test_docente_ajeno_no_puede_calificar(scope_setup: dict) -> None:
    """POST /calificar sobre una entrega de otra comision → 403 (era 201)."""
...
async def test_docente_ajeno_no_puede_recalificar(scope_setup: dict) -> None:
    """PATCH /calificacion sobre una entrega de otra comision → 403 (era 200)."""
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_docente_ajeno_no_puede_devolver(scope_setup: dict) -> None:
    """POST /return sobre una entrega de otra comision → 403 (era 200)."""
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp = await client.post(
...
async def test_rechazo_no_dejo_efectos_en_la_comision_ajena(scope_setup: dict) -> None:
    """El 403 corta ANTES de escribir: la entrega ajena sigue en 'submitted'.
...
async def test_rechazo_es_403_aunque_el_estado_no_sea_calificable(scope_setup: dict) -> None:
...
async def test_docente_sin_ninguna_comision_no_califica(scope_setup: dict) -> None:
    ...
    devolviendo la lista vacia: aca tiene que ser 403, no un pase libre.
    """
